"use strict";
class Intersect {
    constructor(numberOfIntersections, cars_throughput_rate) {
        this.intersects = [];
        this.totalSumOfCars = []; // used to make decisions
    ...
    generateCarArray(n = this.numberOfIntersections) {
        function randomNumberOne2Ten() {
    ...
    decisionAlgo(intersects = this.intersects, lastIndexReserve = this.lastIndexReserve) {
        ...
        const cloneOfIntersects = JSON.parse(JSON.stringify(intersects));
        ...
        if (lastIndexReserve.value !== 0) {
            ...
            this.totalSumOfCars[lastIndexReserve.index] = Math.max(0, lastIndexReserve.value -
        ...
        const indexOfRoadWithMostCars = this.totalSumOfCars.findIndex((ele) => ele === roadWithMostCars);
        console.log(`Index of road with most cars: ${indexOfRoadWithMostCars}`);
        const timeNeeded = Math.min(30, Math.floor(roadWithMostCars / this.cars_throughput_rate));
